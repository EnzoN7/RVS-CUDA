...
__device__ __inline__ encoded_t<channel_t> encodeDepthAndValidity(float depth, float validity)
{
    ...
    {
        union { DepthAndValidity<channel_t> dv; unsigned long long int e; } u;
        u.dv.depth = depth;
...
__device__ __inline__ DepthAndValidity<channel_t> decodeDepthAndValidity(encoded_t<channel_t> encoded)
{
    if constexpr (std::is_same<channel_t, float>::value)
    {
        union { DepthAndValidity<float> dv; unsigned long long int e; } u;
        u.e = encoded;
        return u.dv;
    }
    else if constexpr (std::is_same<channel_t, double>::value)
                             ...
                             int outputWidth)
{
    ...
    channel_t ratio = depth / old.depth;

    if (old.validity * ratio * ratio * ratio <= triValid)
    {
...
(
    ...
    encoded_t<channel_t>*& devOutputDepthValidity, int*& devColorLock,
...
)
{
    ...
    cudaStreamWaitEvent(stream2, initDepthValidity, 0);

    colorizeTriangleDownRightKernel<position_t, color_t, channel_t><<<numBlocks, blockSize, 0, stream2>>>(devInputColor,
                                                                                                          devInputDepth,
                                                                                                          devInputPositions,
                                                                                                          devOutputColor,
                                                                                                          devOutputDepthValidity,
    ...
    if (horizontalWrap)
    {
        ...
        colorizeTriangleHorizontalWarpKernel<position_t, color_t, channel_t><<<numBlocks, blockSize, 0, stream2>>>(devInputColor,
                                                                                                                   devInputDepth,
                                                                                                                   devInputPositions,
                                                                                                                   devOutputColor,
                                                                                                                   devOutputDepthValidity,
    ...
    cudaEventRecord(colorizeTriangles, stream2);

    blockSize = 128;
    numBlocks = (outputImgSize - 1 + blockSize * 2) / (blockSize * 2);

    cudaStreamWaitEvent(stream1, colorizeTriangles, 0);
...
(
    ...
    cudaStream_t& stream1, cudaStream_t& stream2,
    encoded_t<float>*& devOutputDepthValidity, int*& devColorLock,
    cudaEvent_t& synthesizeVirtualDepthValidity, cudaEvent_t& initDepthValidity, cudaEvent_t& colorizeTriangles, cudaEvent_t& projection,
    std::future<void>& futureProjection
);
...
(
    ...
    cudaStream_t& stream1, cudaStream_t& stream2,
    encoded_t<double>*& devOutputDepthValidity, int*& devColorLock,
    cudaEvent_t& synthesizeVirtualDepthValidity, cudaEvent_t& initDepthValidity, cudaEvent_t& colorizeTriangles, cudaEvent_t& projection,
    std::future<void>& futureProjection
);
...
(
    ...
    cudaStream_t& stream1, cudaStream_t& stream2,
    encoded_t<half>*& devOutputDepthValidity, int*& devColorLock,
    cudaEvent_t& synthesizeVirtualDepthValidity, cudaEvent_t& initDepthValidity, cudaEvent_t& colorizeTriangles, cudaEvent_t& projection,
    std::future<void>& futureProjection
);

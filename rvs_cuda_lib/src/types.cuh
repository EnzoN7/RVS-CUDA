...
__device__ __forceinline__ float getY(const float3& pixel) { return pixel.x; }
__device__ __forceinline__ float getU(const float3& pixel) { return pixel.y; }
__device__ __forceinline__ float getV(const float3& pixel) { return pixel.z; }

__device__ __forceinline__ half2 getY(const half3& pixel) { return pixel.Y; }
__device__ __forceinline__ half2 getUV(const half3& pixel) { return pixel.UV; }

__device__ __forceinline__ bool isNaN(float x) { return isnan(x); }
__device__ __forceinline__ bool isNaN(double x) { return isnan(x); }
__device__ __forceinline__ bool isNaN(half x) { return __hisnan(x); }
